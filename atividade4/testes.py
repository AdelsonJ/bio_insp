...
def plota(ntestes):
    ...
    #Emelhor_indv_3 = np.sort(dados[:, 4])
    ...
    plt.plot(geracoes, melhor_indv_1, label="60%", linestyle='', marker='o', color='blue')
    plt.plot(geracoes, melhor_indv_2, label="80%", linestyle='', marker='*', color='red')
    #plt.plot(geracoes, melhor_indv_3, label="100%", linestyle='', marker='.', color='green')
    #plt.plot(geracoes, quando_achou, linestyle='', marker='o', color='red')
    plt.axhline(y=media_valores_1, color='blue', linestyle='-', label=f'Média de 60%: {media_valores_1:.0f}')
    plt.axhline(y=media_valores_2, color='red', linestyle='-', label=f'Média de 80%: {media_valores_2:.0f}')
    #plt.axhline(y=media_valores_3, color='green', linestyle='-', label=f'Média de 100%: {media_valores_3:.0f}')
...
for i in range(ntestes):
    ...
    subprocess.run(["python", arquivo_ag, "30", "2000", "0","0"])

    with open('saida_1.txt', 'r') as arquivo:
        linhas_1 = arquivo.readlines()      

    subprocess.run(["python", arquivo_ag, "50", "2000", "0","1"])
    with open('saida_1.txt', 'r') as arquivo:
